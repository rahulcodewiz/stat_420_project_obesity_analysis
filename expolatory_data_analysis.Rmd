...
p1
```
 On the plot male and Female have equal records count on different age bins. As age grows the number of records are increasing
...
Majority of the people uses public transportation for commute and 2nd is Autombile.
...
***Height, Age, Gender and Weight distributions***
...
p2=ggplot(data = obesity) +
  geom_point(mapping = aes(x = Age, y = Weight,color=Gender))
ggarrange(p1,p2,nrow=3,ncol=1)
```
Observations are:

- Height vs Weight and Height cs Age
...
Last plot shows Height vs weight and mode of transport as label, density of walking people is below 90.
...
Note: we have removed dervied variables agebins, NObeyesdad
...
***AIC and BIC in both directions search***
...
#obs_mod_add_aic_fw=step(lm(Weight~1,data=obs_trn),score=Weight~Gender+Age+Height+family_history_with_overweight+FAVC+FCVC+NCP+CAEC+SMOKE+CH2O+SCC+FAF+TUE+CALC+MTRANS,direction="forward",trace=0)
#mod_performance(obs_mod_add_aic_bw)
...
***Additive model performance analysis***
...
```{r}
#anova(obs_mod_add,obs_mod_full)
```
